...
var humanReadableId = require('human-readable-ids').hri.random;
...
Tunnel.prototype._init = function(cb) {
    ...
    // if no particular subdomain was requested, genereate a human readable one
    var assigned_domain = opt.subdomain ? opt.subdomain : humanReadableId().replace(/-/g, '');
    console.log(assigned_domain);

    // where to quest
    params.uri = base_uri + assigned_domain;
