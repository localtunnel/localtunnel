...
TunnelCluster.prototype.open = function() {
    ...
    function conn_local() {
        ...
        var resolve_home = filepath => filepath.startsWith('~') ? path.join(process.env.HOME, filepath.slice(1)) : filepath;
        var get_local_cert_opts = () => allow_invalid_certificate
            ? { rejectUnauthorized: false }
            : {
                cert: fs.readFileSync(resolve_home(opt.local_cert)),
                key: fs.readFileSync(resolve_home(opt.local_key)),
                ca: opt.local_ca ? [fs.readFileSync(resolve_home(opt.local_ca))] : undefined
            }
